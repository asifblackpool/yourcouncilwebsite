...
namespace RazorPageBusinessWebsite.Helpers.Renderers
{
    ...
    public static class AccordionWithImagesRenderer
    {
        public static IHtmlContent RenderAccordion(this IHtmlHelper htmlHelper,
            string accordionName,
            IEnumerable<AccordionContentWithImages> accordionList)
        {
        ...
        private static TagBuilder CreateAccordionItem(AccordionContentWithImages item, int index)
        {
            ...
            content.InnerHtml.AppendHtml(BuildImages(item));
        ...
        private static string BuildImages(AccordionContentWithImages item)
        {
            StringBuilder sb = new StringBuilder();

            if (item.MultipleImage != null)
            {
                foreach (var image in item.MultipleImage)
                {
                    if (image.Asset != null)
                    {
                        sb.Append(string.Format("<img src='{0}' altText='{1}' />", ImageHelper.GetImageUrl(image), image.AltText));
                    }
                }
            }
